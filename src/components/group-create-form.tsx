		...
		>
			<form {...form.methods} className="flex flex-col gap-2">
				<FormField form={form} name="title" id="title">
					<input
						required
						ref={groupModule.createInputRef}
						className="bg-card border-card"
						placeholder={groupModule.t("form.fields.title.label")}
					/>
				</FormField>
				...
				<button type="submit" className="soft h-10 w-full border-transparent">
					{form.isPending ? <Loader2Icon className="animate-spin" /> : groupModule.t("form.create")}
				</button>
			</form>
		</div>
	);
